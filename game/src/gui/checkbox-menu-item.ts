...
export type CheckboxMenuItemMeta = {
    text: string,
    isChecked: () => boolean,
...
export class CheckboxMenuItem extends MenuItem {
    constructor(private meta: CheckboxMenuItemMeta) {
        super();
    }
    ...
    renderImplContext2d(adapter: DefaultGraphicsAdapter) {
        ...
        let meta = this.meta;
        let isLocked = (meta.isLocked ? meta.isLocked() : false);
        let isChecked = meta.isChecked();
        context.fillStyle = isLocked ? 'grey' :
                      this.isFocused ? 'orange' :
                                       'white';
        let text = meta.text + (isLocked ? this.lockedPostfix : '');
        
        context.textBaseline = 'top';
        context.textAlign = this.horizontalAlign === 'left' ? 'left' : 'center';
        context.font = '24px cambria';
        
        let xx = (this.horizontalAlign === 'left' ? 15 : 0);
        context.fillText(text, xx + 30, 0);
        
        context.strokeStyle = context.fillStyle;
        context.strokeRect(xx + 4, 4, 22, 22);
        if (isChecked) {
            context.fillRect(xx + 6, 6, 18, 18);
        }
